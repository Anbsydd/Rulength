...
//                        System.out.printf("FPS: %.1f%n", fps);
    ...
    private void lerpAndPublish() {
        ...
        // 缩放动画进行中或offset还在追赶target时，使用lerpZoom保持平滑过渡，避免抖动和跳跃
        boolean zoomAnimDone = Math.abs(zoom - targetZoom) < 0.0001;
        boolean offsetAnimDone = Math.abs(offsetX - targetOffsetX) < 0.01 && Math.abs(offsetY - targetOffsetY) < 0.01;
        double offsetLerp = (isDragging && zoomAnimDone && offsetAnimDone) ? lerpDrag : lerpZoom;
